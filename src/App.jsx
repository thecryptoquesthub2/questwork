    ...
    <div style={{ width: '100%', boxSizing: 'border-box' }}>
      ...
      {search ? (
        <div style={{ padding: '0 20px' }}>
          ...
          {filteredGigs.length === 0 ? <div style={{ textAlign: 'center', padding: '40px 20px', color: colors.text2 }}><div style={{ fontSize: '16px', fontWeight: '600', marginBottom: '6px' }}>No gigs found</div><div style={{ fontSize: '13px' }}>Try different keywords</div></div> : <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>{filteredGigs.map(gig => <GigCard key={gig.id} gig={gig} colors={colors} dark={dark} tgUser={tgUser} isPremium={isPremium} bookmarks={bookmarks} toggleBookmark={toggleBookmark} setSelectedGig={setSelectedGig} />)}</div>}
        </div>
      ) : (
        <>
          ...
          <div style={{ padding: '0 20px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>{SAMPLE_GIGS.filter(g => g.featured).map(gig => <GigCard key={gig.id} gig={gig} colors={colors} dark={dark} tgUser={tgUser} isPremium={isPremium} bookmarks={bookmarks} toggleBookmark={toggleBookmark} setSelectedGig={setSelectedGig} />)}</div>
          </div>
...
function GigsPage({ colors, dark, tgUser, isPremium, bookmarks, toggleBookmark, setSelectedGig }) {
  ...
  useEffect(() => {
    const loadGigs = async () => {
      try { const res = await fetch(`${API}/api/gigs`); const data = await res.json(); setDbGigs(data) } catch (err) { console.error(err) }
      setLoading(false)
    }
    loadGigs()
  }, [])
  ...
  return (
    <div style={{ padding: '24px 20px', width: '100%', boxSizing: 'border-box' }}>
      ...
      {loading ? <div style={{ textAlign: 'center', padding: '40px', color: colors.text2 }}>Loading gigs...</div> : <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>{filtered.map(gig => <GigCard key={gig.id} gig={gig} colors={colors} dark={dark} tgUser={tgUser} isPremium={isPremium} bookmarks={bookmarks} toggleBookmark={toggleBookmark} setSelectedGig={setSelectedGig} />)}</div>}
    </div>
  )
}
  ...
  return (
    <div style={{ padding: '24px 20px', width: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        <div><div style={{ fontSize: '12px', fontWeight: '600', color: colors.text2, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: '8px' }}>Role Title *</div><input name="title" value={form.title} onChange={handleChange} placeholder="e.g. Community Manager" style={input} /></div>
        <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
          <div><div style={{ fontSize: '12px', fontWeight: '600', color: colors.text2, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: '8px' }}>Budget (USDT) *</div><input name="pay_usdt" value={form.pay_usdt} onChange={handleChange} type="text" placeholder="500 or Negotiable" style={input} /></div>
          <div><div style={{ fontSize: '12px', fontWeight: '600', color: colors.text2, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: '8px' }}>Duration</div><select name="duration" value={form.duration} onChange={handleChange} style={{ ...input, appearance: 'none' }}><option>1 Week</option><option>1 Month</option><option>3 Months</option><option>Ongoing</option></select></div>
        </div>
        <div><div style={{ fontSize: '12px', fontWeight: '600', color: colors.text2, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: '8px' }}>Region</div><select name="region" value={form.region} onChange={handleChange} style={{ ...input, appearance: 'none' }}><option>Global</option><option>Africa</option><option>MENA</option><option>Remote</option></select></div>
...
function SearchPage({ colors, dark, tgUser }) {
  ...
  const searchGigs = (value) => {
    ...
    const lower = value.toLowerCase()
    setGigResults(SAMPLE_GIGS.filter(g => g.title.toLowerCase().includes(lower) || g.company.toLowerCase().includes(lower) || g.category.toLowerCase().includes(lower) || (g.tag || '').toLowerCase().includes(lower)))
  }
  const searchDbUsers = async (value) => {
    if (!value.trim()) { setDbUsers([]); return }
    setLoading(true)
    try { const res = await fetch(`${API}/api/users/search?q=${encodeURIComponent(value)}`); const data = await res.json(); setDbUsers(Array.isArray(data) ? data : []) } catch (err) { console.error(err) }
    setLoading(false)
  }
  const handleSearch = (value) => { setQuery(value); if (mode === 'gigs') searchGigs(value); else searchDbUsers(value) }
    ...
    <div style={{ padding: '24px 20px', width: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'flex', gap: '8px', marginBottom: '16px' }}>
        {[{ key: 'freelancers', label: 'Freelancers' }, { key: 'gigs', label: 'Gigs' }].map(m => (
          <button key={m.key} onClick={() => { haptic('light'); setMode(m.key); setQuery(''); setGigResults([]); setDbUsers([]) }} style={{ flex: 1, padding: '11px', borderRadius: '12px', background: mode === m.key ? colors.btnBg : colors.surface2, border: `1px solid ${mode === m.key ? colors.btnBg : colors.border}`, color: mode === m.key ? colors.btnText : colors.text2, fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>{m.label}</button>
        ))}
      </div>
        ...
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
            {filteredFreelancers.map(f => (
              <div key={f.id} onClick={() => { haptic('light'); setSelectedFreelancer(f) }} style={{ background: colors.card, border: `1px solid ${colors.border}`, borderRadius: '16px', padding: '16px', cursor: 'pointer' }}>
                ...
                <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>{f.skills.map((s, i) => <div key={i} style={{ fontSize: '11px', padding: '3px 10px', borderRadius: '20px', background: colors.accentBg, border: `1px solid ${colors.accentBorder}`, color: colors.accent }}>{s}</div>)}</div>
              </div>
            ))}
          </div>
        </>
        ...
        <>
          ...
          {query && !loading && (
            <div>
              ...
              {gigResults.length === 0 ? <div style={{ textAlign: 'center', padding: '40px 20px', color: colors.text2 }}><div style={{ fontSize: '16px', fontWeight: '600', marginBottom: '6px' }}>No gigs found</div><div style={{ fontSize: '13px' }}>Try different keywords</div></div> : <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>{gigResults.map(gig => <div key={gig.id} style={{ background: colors.card, border: `1px solid ${colors.border}`, borderRadius: '16px', padding: '16px' }}><div style={{ fontSize: '15px', fontWeight: '600', marginBottom: '3px' }}>{gig.title}</div><div style={{ fontSize: '13px', color: colors.text2, marginBottom: '8px' }}>{gig.company} · {gig.pay}</div><div style={{ fontSize: '11px', padding: '3px 9px', borderRadius: '6px', background: colors.surface2, color: colors.text2, display: 'inline-block' }}>{gig.category}</div></div>)}</div>}
            </div>
          )}
        </>
      )}
...
function ApplicationsReceived({ colors, dark, tgUser }) {
  ...
  useEffect(() => {
    const load = async () => {
      if (!tgUser?.id) { setLoading(false); return }
      try { const res = await fetch(`${API}/api/applications/received/${tgUser.id}`); const data = await res.json(); setApplications(Array.isArray(data) ? data : []) } catch (err) { console.error(err) }
      setLoading(false)
    }
    load()
  }, [tgUser])
  if (loading) return <div style={{ textAlign: 'center', padding: '30px', color: colors.text2 }}>Loading applications...</div>
  if (applications.length === 0) return <div style={{ textAlign: 'center', padding: '40px 20px', color: colors.text2 }}><div style={{ fontSize: '16px', fontWeight: '600', marginBottom: '6px' }}>No applications yet</div><div style={{ fontSize: '13px' }}>When someone applies to your gigs, they will appear here</div></div>
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
      {applications.map((app, i) => (
        <div key={i} style={{ background: colors.card, border: `1px solid ${colors.border}`, borderRadius: '16px', overflow: 'hidden' }}>
          <div onClick={() => setExpanded(expanded === i ? null : i)} style={{ padding: '16px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div><div style={{ fontSize: '15px', fontWeight: '600', marginBottom: '3px' }}>{app.gig_title || 'Gig Application'}</div><div style={{ fontSize: '13px', color: colors.text2 }}>@{app.applicant_username || 'unknown'}</div></div>
    ...
    <div style={{ padding: '24px 20px', width: '100%', boxSizing: 'border-box' }}>
      <div style={{ background: colors.card, border: `1px solid ${colors.border}`, borderRadius: '20px', padding: '20px', marginBottom: '16px', boxShadow: dark ? 'none' : '0 2px 12px rgba(0,0,0,0.06)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '16px' }}>
          {tgUser?.photo_url ? <img src={tgUser.photo_url} style={{ width: '64px', height: '64px', borderRadius: '50%', border: `2px solid ${colors.border}`, objectFit: 'cover', flexShrink: 0 }} alt="avatar" /> : <div style={{ width: '64px', height: '64px', borderRadius: '50%', background: dark ? 'rgba(255,255,255,0.1)' : 'rgba(0,0,0,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '26px', fontWeight: '700', flexShrink: 0, border: `2px solid ${colors.border}` }}>{tgUser ? tgUser.first_name?.[0] : '?'}</div>}
          <div style={{ flex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: '6px', marginTop: '8px', flexWrap: 'wrap' }}>
              <div style={{ fontSize: '11px', padding: '3px 10px', borderRadius: '20px', background: isPremium ? 'rgba(245,200,66,0.15)' : colors.accentBg, color: isPremium ? '#F5C842' : colors.accent, fontWeight: '600', border: `1px solid ${isPremium ? 'rgba(245,200,66,0.3)' : colors.accentBorder}` }}>{isPremium ? '⭐ Premium' : 'Free Plan'}</div>
              <div style={{ fontSize: '11px', padding: '3px 10px', borderRadius: '20px', background: savedAvailability === 'Available' ? 'rgba(52,211,153,0.1)' : colors.surface2, color: savedAvailability === 'Available' ? '#34d399' : colors.text2, fontWeight: '500', border: `1px solid ${savedAvailability === 'Available' ? 'rgba(52,211,153,0.3)' : colors.border}` }}>{savedAvailability}</div>
            </div>
          </div>
        </div>
        ...
        {backupEmail ? (
          <div style={{ marginBottom: '12px', padding: '10px 12px', borderRadius: '10px', background: 'rgba(52,211,153,0.07)', border: '1px solid rgba(52,211,153,0.2)', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div><div style={{ fontSize: '10px', fontWeight: '600', color: '#34d399', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Backup Login</div><div style={{ fontSize: '13px', color: colors.text, marginTop: '1px' }}>{backupEmail}</div></div>
          ...
          <>
            {!isEditing ? (
              ...
              <>
                ...
                <div style={{ marginBottom: '14px' }}><div style={{ fontSize: '12px', fontWeight: '600', color: colors.text2, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: '8px' }}>About Me</div><textarea value={bio} onChange={e => setBio(e.target.value)} placeholder="Tell clients about yourself..." style={{ width: '100%', height: '80px', padding: '12px 14px', borderRadius: '12px', resize: 'none', background: colors.surface2, border: `1px solid ${colors.border}`, color: colors.text, fontSize: '14px', fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }} /></div>
                <div style={{ marginBottom: '14px' }}><div style={{ fontSize: '12px', fontWeight: '600', color: colors.text2, letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: '8px' }}>Skills (comma separated)</div><input value={skills} onChange={e => setSkills(e.target.value)} placeholder="e.g. Community Management, BD, Writing..." style={{ width: '100%', padding: '12px 14px', borderRadius: '12px', background: colors.surface2, border: `1px solid ${colors.border}`, color: colors.text, fontSize: '14px', fontFamily: 'inherit', outline: 'none', boxSizing: 'border-box' }} /></div>
                <div style={{ display: 'flex', gap: '10px' }}>
        ...
        {activeTab === 'bookmarks' && (
          <div>
            {bookmarkedGigs.length === 0 ? <div style={{ textAlign: 'center', padding: '30px 20px', color: colors.text2 }}><div style={{ fontSize: '24px', marginBottom: '8px' }}>🔖</div><div style={{ fontSize: '15px', fontWeight: '600', marginBottom: '6px' }}>No saved gigs yet</div><div style={{ fontSize: '13px' }}>Tap the bookmark icon on any gig to save it here</div></div> : <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>{bookmarkedGigs.map(gig => <div key={gig.id} style={{ background: colors.surface2, border: `1px solid ${colors.border}`, borderRadius: '14px', padding: '14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><div style={{ flex: 1 }}><div style={{ fontSize: '14px', fontWeight: '600', marginBottom: '2px' }}>{gig.title}</div><div style={{ fontSize: '12px', color: colors.text2 }}>{gig.company} · {gig.pay}</div></div><div style={{ color: '#C8A95A' }}><IconBookmark size={16} filled /></div></div>)}</div>}
          </div>
        )}
        ...
        <div onClick={() => { haptic('light'); setShowPayment(false) }} style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, zIndex: 9999, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(8px)', WebkitBackdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', boxSizing: 'border-box' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: dark ? '#1c1c1e' : '#ffffff', borderRadius: '24px', padding: '28px 24px 32px', width: '100%', maxWidth: '420px', border: `1px solid ${colors.border}`, boxShadow: '0 20px 60px rgba(0,0,0,0.3)', boxSizing: 'border-box', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '20px' }}>
              {['card', 'usdt'].map(m => <button key={m} onClick={() => setPaymentMethod(m)} style={{ flex: 1, padding: '10px', borderRadius: '12px', background: paymentMethod === m ? colors.btnBg : colors.surface2, border: `1px solid ${paymentMethod === m ? colors.btnBg : colors.border}`, color: paymentMethod === m ? colors.btnText : colors.text2, fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>{m === 'card' ? 'Credit Card' : 'USDT'}</button>)}
            </div>
            {paymentMethod === 'card' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginBottom: '16px' }}>
                ...
                <button onClick={async () => {
                  ...
                  setProcessing(false)
                }} style={{ width: '100%', padding: '16px', borderRadius: '14px', background: processing ? colors.surface2 : '#F5C842', border: 'none', color: '#000000', fontSize: '16px', fontWeight: '700', cursor: processing ? 'not-allowed' : 'pointer' }}>{processing ? 'Processing...' : 'Pay $15/month'}</button>
