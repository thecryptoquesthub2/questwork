...
// Ink #0B1220 | Paper #F4F1EA | Gold #C8A95A | Red #FF5A3C | Green #4ADE80
...
const SAMPLE_FREELANCERS = [
  { id: 1, name: "Amara Okonkwo", username: "amara_web3", skills: ["Community Management", "Discord"], questScore: 4.9, gigsCompleted: 52, earned: "$28K", available: true, bio: "Web3 community builder with 4+ years growing crypto projects globally." },
  { id: 2, name: "Hassan Al-Rashid", username: "hassan_bd", skills: ["Business Development", "Partnerships"], questScore: 4.7, gigsCompleted: 38, earned: "$19K", available: true, bio: "BD expert connecting Web3 projects with key partners worldwide." },
...
function SignupModal({ colors, dark, onSave }) {
  ...
  const handleSave = async () => {
    ...
    setSaving(true)
    try {
      await fetch(`${API}/api/users/backup-login`, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, password }) })
    } catch (_) {}
...
function WorkSubmitModal({ contract, tgUser, colors, dark, onClose, onSubmitted }) {
  ...
  const handleSubmit = async () => {
    if (!note.trim() && !linkUrl.trim()) { alert('Please add a note or link describing your work.'); return }
    setSubmitting(true)
    try {
      await fetch(`${API}/api/submissions`, {
        ...
        body: JSON.stringify({
          ...
          gig_title: contract.gig_title,
          client_tg_id: contract.client_tg_id,
          freelancer_tg_id: contract.freelancer_tg_id,
          note,
          link_url: linkUrl,
        })
      })
    ...
    } catch (_) { alert('Error submitting work. Please try again.') }
    setSubmitting(false)
  }
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.7)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: dark ? '#0B1220' : '#fff', borderRadius: '24px 24px 0 0', width: '100%', maxWidth: '500px', padding: '24px 20px 40px', border: `1px solid ${colors.border}`, boxShadow: '0 -10px 40px rgba(0,0,0,0.3)' }}>
        <div style={{ width: '40px', height: '4px', borderRadius: '2px', background: colors.border, margin: '0 auto 20px' }} />
        ...
        {done ? (
          <div style={{ textAlign: 'center', padding: '24px 0' }}>
            ...
            <div style={{ fontSize: '13px', color: colors.text2 }}>The client has been notified and will review your submission.</div>
          </div>
        ) : (
          <>
            <div style={{ fontSize: '18px', fontWeight: '700', color: colors.text, marginBottom: '4px' }}>Submit Work</div>
            <div style={{ fontSize: '13px', color: colors.text2, marginBottom: '20px' }}>For: {contract.gig_title || 'Gig Contract'} · {contract.amount_ton} TON locked</div>

            <div style={{ marginBottom: '14px' }}>
              <div style={{ fontSize: '11px', fontWeight: '600', color: colors.text2, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '8px' }}>Work Description / Note *</div>
              <textarea value={note} onChange={e => setNote(e.target.value)} placeholder="Describe what you delivered, any context the client needs..." style={{ ...inp, height: '100px', resize: 'none' }} />
            </div>

            <div style={{ marginBottom: '24px' }}>
              <div style={{ fontSize: '11px', fontWeight: '600', color: colors.text2, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '8px' }}>Portfolio / Delivery Link</div>
            ...
            <div style={{ padding: '12px', borderRadius: '12px', background: colors.goldBg, border: `1px solid ${colors.goldBorder}`, marginBottom: '20px', fontSize: '12px', color: colors.gold, lineHeight: 1.6 }}>
              💡 After you submit, the client reviews your work and releases payment from escrow.
            </div>
...
function EscrowModal({ freelancer, tgUser, colors, dark, onClose, onFunded }) {
  ...
  const handleFund = async () => {
    ...
    try {
      await fetch(`${API}/api/escrow`, {
        ...
        body: JSON.stringify({
          ...
          freelancer_tg_id: String(freelancer.tg_id || freelancer.id),
          gig_title: `Contract with ${freelancer.name || freelancer.first_name}`,
    ...
    } catch (_) { alert('Error recording contract. Please try again.') }
    setFunding(false)
  }
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.75)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: dark ? '#0B1220' : '#fff', borderRadius: '24px 24px 0 0', width: '100%', maxWidth: '500px', padding: '24px 20px 44px', border: `1px solid ${colors.border}`, boxShadow: '0 -10px 40px rgba(0,0,0,0.4)', maxHeight: '90vh', overflowY: 'auto' }}>
        <div style={{ width: '40px', height: '4px', borderRadius: '2px', background: colors.border, margin: '0 auto 20px' }} />
        ...
        {done ? (
          <div style={{ textAlign: 'center', padding: '24px 0' }}>
            ...
            <div style={{ fontSize: '13px', color: colors.text2 }}>Funds are locked. The freelancer has been notified to begin work.</div>
          </div>
        ) : (
          <>
            ...
            {step === 1 && (
              <>
                ...
                {amountTon && parseFloat(amountTon) > 0 && (
                  <div style={{ background: colors.goldBg, border: `1px solid ${colors.goldBorder}`, borderRadius: '14px', padding: '16px', marginBottom: '20px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                      <span style={{ fontSize: '13px', color: colors.text2 }}>Total locked</span>
                      <span style={{ fontSize: '13px', fontWeight: '600', color: colors.text }}>{amountTon} TON</span>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px' }}>
                      <span style={{ fontSize: '13px', color: colors.text2 }}>QuestWork (10%)</span>
                      <span style={{ fontSize: '13px', color: colors.gold }}>{commission} TON</span>
                    </div>
                ...
                <button onClick={() => { if (!amountTon || parseFloat(amountTon) <= 0) { alert('Enter a valid TON amount.'); return } setStep(2) }} style={{ width: '100%', padding: '15px', borderRadius: '13px', background: colors.btnBg, border: 'none', color: colors.btnText, fontSize: '16px', fontWeight: '700', cursor: 'pointer' }}>
                  Continue →
                </button>
              </>
            )}
            ...
            {step === 2 && (
              <>
                <div style={{ background: colors.surface2, border: `1px solid ${colors.border}`, borderRadius: '14px', padding: '16px', marginBottom: '16px' }}>
                  <div style={{ fontSize: '12px', fontWeight: '600', color: colors.text2, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '10px' }}>Send {amountTon} TON to this address</div>
                  <div style={{ fontSize: '13px', fontFamily: 'monospace', color: colors.text, wordBreak: 'break-all', lineHeight: 1.7, padding: '10px', background: dark ? 'rgba(0,0,0,0.3)' : 'rgba(0,0,0,0.04)', borderRadius: '10px', marginBottom: '10px' }}>
                    {TON_CONTRACT}
                  </div>
                  <button onClick={() => { navigator.clipboard.writeText(TON_CONTRACT); haptic('medium') }} style={{ width: '100%', padding: '10px', borderRadius: '10px', background: colors.goldBg, border: `1px solid ${colors.goldBorder}`, color: colors.gold, fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>
                    Copy Address
                  </button>
                </div>
                <div style={{ fontSize: '12px', color: colors.text2, lineHeight: 1.6, marginBottom: '20px', padding: '12px', borderRadius: '12px', background: colors.surface2, border: `1px solid ${colors.border}` }}>
                  Open Tonkeeper or your TON wallet, send exactly <strong style={{ color: colors.text }}>{amountTon} TON</strong> to the address above, then paste your transaction hash below.
                </div>
...
// ─────────────────────────────────────────────────────────────────────────────
// CONTRACT CARD — inline in chat
// ─────────────────────────────────────────────────────────────────────────────
function ContractCard({ contract, tgUser, colors, dark, onWorkSubmit, onRelease }) {
  ...
  return (
    <div style={{ background: dark ? 'rgba(200,169,90,0.07)' : 'rgba(200,169,90,0.05)', border: `1px solid ${colors.goldBorder}`, borderRadius: '16px', padding: '14px', margin: '8px 0' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px', marginBottom: '12px' }}>
        ...
        <div style={{ background: colors.surface2, borderRadius: '10px', padding: '10px', border: `1px solid ${colors.border}` }}>
          <div style={{ fontSize: '10px', color: colors.text2, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '3px' }}>Contract</div>
          <div style={{ fontSize: '11px', fontFamily: 'monospace', color: colors.text2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{TON_CONTRACT.slice(0,14)}…</div>
        </div>
      </div>

      {contract.gig_title && (
        <div style={{ fontSize: '12px', color: colors.text2, marginBottom: '12px' }}>Gig: {contract.gig_title}</div>
      )}

      {/* Freelancer: submit work button */}
      {isFreelancer && contract.status === 'funded' && (
        <button onClick={() => onWorkSubmit?.(contract)} style={{ width: '100%', padding: '11px', borderRadius: '11px', background: colors.btnBg, border: 'none', color: colors.btnText, fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}>
          Submit Work →
        </button>
      )}
      ...
      {isClient && contract.status === 'submitted' && (
        <button onClick={handleRelease} disabled={releasing} style={{ width: '100%', padding: '11px', borderRadius: '11px', background: releasing ? colors.surface2 : colors.green, border: 'none', color: '#0B1220', fontSize: '13px', fontWeight: '700', cursor: releasing ? 'not-allowed' : 'pointer' }}>
          {releasing ? 'Releasing...' : '✓ Approve & Release Payment'}
        </button>
      )}

      {contract.status === 'released' && (
        <div style={{ textAlign: 'center', fontSize: '12px', color: colors.green, fontWeight: '600' }}>Payment has been released to the freelancer.</div>
      )}
    </div>
  )
}
...
// ─────────────────────────────────────────────────────────────────────────────
function MessagingScreen({ tgUser, colors, dark, initialRecipient, onClose }) {
  ...
  const [showWorkSubmit, setShowWorkSubmit] = useState(null) // contract obj
  ...
  // Load messages + contracts for active thread
  ...
  useEffect(() => {
    if (activeThread) {
      ...
      pollRef.current = setInterval(() => loadThread(activeThread), 5000)
    }
  ...
  // Open initial recipient if passed
  useEffect(() => {
    if (initialRecipient) setActiveThread(initialRecipient)
  }, [initialRecipient])
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      if (data.error === 'blocked') {
        setMsgError(data.reason || 'Message blocked: contact info detected.')
        setBlocked(true)
  ...
  // Combined timeline: messages + contract cards, sorted by time
  const timeline = [
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 8000, background: dark ? '#000' : '#F4F1EA', display: 'flex', flexDirection: 'column' }}>
      ...
      <div style={{ padding: '16px 20px 14px', background: dark ? 'rgba(11,18,32,0.97)' : 'rgba(255,255,255,0.97)', backdropFilter: 'blur(20px)', borderBottom: `1px solid ${colors.border}`, display: 'flex', alignItems: 'center', gap: '12px', flexShrink: 0 }}>
        ...
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: '17px', fontWeight: '700', color: colors.text }}>
            {activeThread ? (activeThread.other_name || 'Chat') : 'Messages'}
          </div>
          {activeThread && <div style={{ fontSize: '12px', color: colors.text2 }}>@{activeThread.other_username || ''}</div>}
        </div>
        {activeThread && (
      ...
      {!activeThread && (
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px 16px' }}>
          ...
          {threads.map((t, i) => (
            <div key={i} onClick={() => openThread(t)} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px', borderRadius: '16px', background: colors.card, border: `1px solid ${colors.border}`, marginBottom: '8px', cursor: 'pointer', boxShadow: dark ? 'none' : '0 1px 6px rgba(0,0,0,0.05)' }}>
              ...
              {t.unread_count > 0 && (
                <div style={{ width: '20px', height: '20px', borderRadius: '50%', background: colors.red, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '11px', fontWeight: '700', color: '#fff', flexShrink: 0 }}>
        ...
        <>
            ...
            {timeline.map((item, i) => {
              if (item.type === 'contract') {
                return (
                  <ContractCard
                    ...
                    colors={colors}
                    dark={dark}
                    onWorkSubmit={(c) => setShowWorkSubmit(c)}
                    onRelease={() => loadThread(activeThread)}
                  />
                )
              }
              const m = item.data
              ...
              return (
                <div key={i} style={{ display: 'flex', justifyContent: isMine ? 'flex-end' : 'flex-start', marginBottom: '2px' }}>
                  <div style={{ maxWidth: '78%', padding: '10px 14px', borderRadius: isMine ? '18px 18px 4px 18px' : '18px 18px 18px 4px', background: isMine ? colors.btnBg : colors.card, color: isMine ? colors.btnText : colors.text, fontSize: '14px', lineHeight: 1.55, border: isMine ? 'none' : `1px solid ${colors.border}`, boxShadow: dark ? 'none' : '0 1px 4px rgba(0,0,0,0.06)' }}>
                    {m.content}
                  </div>
                </div>
              )
          ...
          <div style={{ padding: '10px 16px 20px', borderTop: `1px solid ${colors.border}`, background: dark ? 'rgba(11,18,32,0.97)' : 'rgba(255,255,255,0.97)', backdropFilter: 'blur(20px)', display: 'flex', gap: '8px', flexShrink: 0 }}>
            <input
              ...
              style={{ flex: 1, padding: '12px 16px', borderRadius: '22px', background: colors.surface2, border: `1px solid ${blocked ? 'rgba(255,90,60,0.4)' : colors.border}`, color: colors.text, fontSize: '14px', fontFamily: 'inherit', outline: 'none', transition: 'border-color 0.2s' }}
            />
            <button onClick={sendMessage} disabled={sending || !newMsg.trim()} style={{ width: '44px', height: '44px', borderRadius: '50%', background: newMsg.trim() ? colors.btnBg : colors.surface2, border: 'none', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: newMsg.trim() ? 'pointer' : 'default', color: newMsg.trim() ? colors.btnText : colors.text2, flexShrink: 0, transition: 'background 0.2s' }}>
              <IconSend size={17}/>
            </button>
          </div>
        </>
      ...
      {showEscrow && activeThread && (
        <EscrowModal
          freelancer={{ tg_id: activeThread.other_id, name: activeThread.other_name }}
          tgUser={tgUser}
          colors={colors}
          dark={dark}
          onClose={() => setShowEscrow(false)}
          onFunded={() => loadThread(activeThread)}
        />
      )}
      {showWorkSubmit && (
        <WorkSubmitModal
          contract={showWorkSubmit}
          tgUser={tgUser}
          colors={colors}
          dark={dark}
          onClose={() => setShowWorkSubmit(null)}
          onSubmitted={() => loadThread(activeThread)}
        />
      )}
    </div>
  )
...
// FREELANCER MODAL
// ─────────────────────────────────────────────────────────────────────────────
function FreelancerModal({ freelancer, colors, dark, tgUser, onClose, onMessage }) {
  return (
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: dark ? '#0B1220' : '#fff', borderRadius: '24px 24px 0 0', width: '100%', maxWidth: '500px', maxHeight: '80vh', overflowY: 'auto', border: `1px solid ${colors.border}`, boxShadow: '0 -10px 40px rgba(0,0,0,0.3)' }}>
        ...
        <div style={{ padding: '20px 20px 40px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '20px' }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: '18px', fontWeight: '700', marginBottom: '2px', color: colors.text }}>{freelancer.name}</div>
          ...
          <div style={{ marginBottom: '24px' }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
              {freelancer.skills.map((s, i) => (
                <div key={i} style={{ fontSize: '12px', padding: '5px 12px', borderRadius: '20px', background: colors.goldBg, border: `1px solid ${colors.goldBorder}`, color: colors.gold, fontWeight: '500' }}>{s}</div>
              ))}
            </div>
          </div>
      ...
      {search ? (
        <div style={{ padding: '0 20px' }}>
          ...
          {filteredGigs.length === 0
            ? <div style={{ textAlign: 'center', padding: '40px 20px', color: colors.text2 }}><div style={{ fontSize: '16px', fontWeight: '600', marginBottom: '6px', color: colors.text }}>No gigs found</div><div style={{ fontSize: '13px' }}>Try different keywords</div></div>
    ...
    <div style={{ padding: '24px 20px', width: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'flex', gap: '8px', overflowX: 'auto', paddingBottom: '16px' }}>
        {labels.map((lab, i) => {
          ...
          return (
            <button key={lab} onClick={() => { haptic('light'); setFilter(cat) }} style={{ padding: '7px 14px', borderRadius: '20px', whiteSpace: 'nowrap', background: filter === cat ? colors.btnBg : colors.surface2, border: `1px solid ${filter === cat ? colors.btnBg : colors.border}`, color: filter === cat ? colors.btnText : colors.text, fontSize: '13px', fontWeight: '500', cursor: 'pointer' }}>{lab}</button>
          )
        })}
      </div>
      {loading
        ? <div style={{ textAlign: 'center', padding: '40px', color: colors.text2 }}>Loading gigs...</div>
        : <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>{filtered.map(g => <GigCard key={g.id} gig={g} colors={colors} dark={dark} bookmarks={bookmarks} toggleBookmark={toggleBookmark} setSelectedGig={setSelectedGig}/>)}</div>
      }
    </div>
  )
}
  ...
  return (
    <div style={{ padding: '24px 20px', width: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
        ...
        <button onClick={handleSubmit} disabled={loading} style={{ width: '100%', padding: '16px', borderRadius: '14px', marginTop: '8px', background: loading ? colors.surface2 : colors.btnBg, border: 'none', color: loading ? colors.text2 : colors.btnText, fontSize: '16px', fontWeight: '700', cursor: loading ? 'not-allowed' : 'pointer' }}>
...
// SEARCH PAGE
// ─────────────────────────────────────────────────────────────────────────────
function SearchPage({ colors, dark, tgUser, onMessage }) {
...
function ApplicationsReceived({ colors, dark, tgUser }) {
  ...
  if (!applications.length) return (
    <div style={{ textAlign: 'center', padding: '40px 20px', color: colors.text2 }}>
      <div style={{ fontSize: '16px', fontWeight: '600', marginBottom: '6px', color: colors.text }}>No applications yet</div>
      ...
      {applications.map((app, i) => (
        <div key={i} style={{ background: colors.card, border: `1px solid ${colors.border}`, borderRadius: '16px', overflow: 'hidden' }}>
          <div onClick={() => setExpanded(expanded === i ? null : i)} style={{ padding: '16px', cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            <div>
              <div style={{ fontSize: '15px', fontWeight: '600', marginBottom: '3px', color: colors.text }}>{app.gig_title || 'Gig Application'}</div>
          ...
          {expanded === i && (
            <div style={{ padding: '0 16px 16px', borderTop: `1px solid ${colors.border}` }}>
              ...
              <button onClick={() => { haptic('medium'); window.open(`https://t.me/${app.applicant_username}`, '_blank') }} style={{ width: '100%', marginTop: '14px', padding: '11px', borderRadius: '12px', background: colors.btnBg, border: 'none', color: colors.btnText, fontSize: '13px', fontWeight: '600', cursor: 'pointer' }}>Message on Telegram</button>
            </div>
          )}
        </div>
      ))}
...
function ProfilePage({ colors, dark, tgUser, isPremium, setIsPremium, notificationsOn, setNotificationsOn, cycleTheme, theme, bookmarks, backupEmail, setShowSignup }) {
  ...
  const handleSave = async () => {
    ...
    if (tgUser?.id) {
      try {
        await fetch(`${API}/api/users/${tgUser.id}`, { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ bio, skills, availability, dm_enabled: dmEnabled }) })
      } catch (_) {}
    }
  }
  ...
  const sections = [
    { title: 'Preferences', items: [
      ...
      { label: 'DM (Open to Messages)', value: dmEnabled ? 'On' : 'Off', arrow: true, action: async () => {
  ...
  return (
    <div style={{ padding: '24px 20px', width: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ background: colors.card, border: `1px solid ${colors.border}`, borderRadius: '20px', padding: '20px', marginBottom: '16px', boxShadow: dark ? 'none' : '0 2px 12px rgba(0,0,0,0.06)' }}>
        ...
        {/* QuestScore */}
        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '12px 14px', borderRadius: '14px', background: colors.goldBg, border: `1px solid ${colors.goldBorder}`, marginBottom: '12px' }}>
          <div style={{ fontSize: '24px', fontWeight: '700', color: colors.gold }}>{questScore}</div>
          <div>
            <div style={{ fontSize: '12px', fontWeight: '700', color: colors.gold }}>QuestScore</div>
            <div style={{ fontSize: '11px', color: colors.goldDeep }}>{jobsDone} gig{jobsDone !== 1 ? 's' : ''} completed · grows with each release</div>
          </div>
          <div style={{ marginLeft: 'auto' }}><IconStar size={20}/></div>
        </div>
        ...
        {savedSkills && !isEditing && (
          <div style={{ marginBottom: '14px', display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
            {savedSkills.split(',').map((s, i) => <div key={i} style={{ fontSize: '11px', padding: '4px 10px', borderRadius: '20px', background: colors.goldBg, border: `1px solid ${colors.goldBorder}`, color: colors.gold, fontWeight: '500' }}>{s.trim()}</div>)}
          </div>
        )}
          ...
          !isEditing ? (
            ...
            <>
              ...
              <div style={{ marginBottom: '14px' }}>
                ...
                <textarea value={bio} onChange={e => setBio(e.target.value)} placeholder="Tell clients about yourself..." style={{ ...inp, height: '80px', resize: 'none' }} />
              </div>
              <div style={{ marginBottom: '14px' }}>
                ...
                <input value={skills} onChange={e => setSkills(e.target.value)} placeholder="e.g. Community Management, BD, Writing..." style={inp} />
              </div>
              <div style={{ display: 'flex', gap: '10px' }}>
                <button onClick={() => { haptic('light'); setBio(savedBio); setSkills(savedSkills); setAvailability(savedAvailability); setIsEditing(false) }} style={{ flex: 1, padding: '13px', borderRadius: '12px', background: colors.surface2, border: `1px solid ${colors.border}`, color: colors.text, fontSize: '14px', fontWeight: '600', cursor: 'pointer' }}>Cancel</button>
      ...
      {!isPremium && (
        <div onClick={() => { haptic('medium'); setShowPayment(true) }} style={{ background: dark ? 'rgba(200,169,90,0.06)' : 'rgba(200,169,90,0.04)', border: `1px solid ${colors.goldBorder}`, borderRadius: '16px', padding: '16px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '16px', cursor: 'pointer' }}>
          <div>
            <div style={{ fontSize: '14px', fontWeight: '700', color: colors.text }}>Upgrade to Premium</div>
      ...
      {/* Founder card */}
      <div style={{ background: colors.card, border: `1px solid ${colors.border}`, borderRadius: '20px', padding: '20px', marginTop: '8px', marginBottom: '20px' }}>
        ...
        <div onClick={() => setShowPayment(false)} style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'rgba(0,0,0,0.65)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px', boxSizing: 'border-box' }}>
          <div onClick={e => e.stopPropagation()} style={{ background: dark ? '#0B1220' : '#fff', borderRadius: '24px', padding: '28px 24px 32px', width: '100%', maxWidth: '420px', border: `1px solid ${colors.border}`, boxShadow: '0 20px 60px rgba(0,0,0,0.3)', boxSizing: 'border-box', maxHeight: '90vh', overflowY: 'auto' }}>
            ...
            <div style={{ background: colors.surface2, borderRadius: '14px', padding: '14px', marginBottom: '20px', border: `1px solid ${colors.border}` }}>
              {['AI Pitch Writer', 'Priority Visibility', 'Premium Badge', 'Advanced Analytics', 'AI Job Matching', 'Direct Client Invites', 'Unlimited Applications'].map((f, i, arr) => (
            ...
            {paymentMethod === 'card' && (
              <div>
                <div style={{ padding: '20px', background: colors.surface2, borderRadius: '14px', marginBottom: '16px', border: `1px solid ${colors.border}`, textAlign: 'center', color: colors.text2, fontSize: '13px' }}>
                  Card payments via Braintree — contact @QuestWorkSupport to set up.
                </div>
  ...
  return (
    <div style={{ backgroundColor: colors.bg, minHeight: '100vh', width: '100vw', maxWidth: '100vw', margin: 0, padding: 0, color: colors.text, fontFamily: '-apple-system, BlinkMacSystemFont, "SF Pro Display", "Helvetica Neue", sans-serif', WebkitFontSmoothing: 'antialiased', position: 'relative', overflowX: 'hidden', transition: 'background-color 0.3s ease', boxSizing: 'border-box' }}>
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 50, padding: '16px 20px 12px', background: dark ? 'rgba(0,0,0,0.92)' : 'rgba(244,241,234,0.92)', backdropFilter: 'blur(30px)', WebkitBackdropFilter: 'blur(30px)', borderBottom: `1px solid ${colors.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', boxSizing: 'border-box' }}>
        ...
        <button onClick={() => openMessaging()} style={{ position: 'relative', width: '40px', height: '40px', borderRadius: '50%', background: colors.surface2, border: `1px solid ${colors.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: colors.text }}>
          <IconMessage size={20}/>
          {unreadCount > 0 && (
            <div style={{ position: 'absolute', top: '1px', right: '1px', width: '16px', height: '16px', borderRadius: '50%', background: colors.red, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', fontWeight: '700', color: '#fff', border: `2px solid ${dark ? '#000' : '#F4F1EA'}` }}>
      ...
      <div style={{ opacity: animating ? 0 : 1, transform: animating ? 'translateY(6px)' : 'translateY(0)', transition: 'opacity 0.15s ease, transform 0.15s ease', paddingBottom: '80px', position: 'relative', zIndex: 1, width: '100%', boxSizing: 'border-box' }}>
        ...
        {active === 'search'  && <SearchPage  colors={colors} dark={dark} tgUser={tgUser} onMessage={openMessaging}/>}
      ...
      {showMessages && (
        <MessagingScreen
          ...
          onClose={() => { setShowMessages(false); setMessagingRecipient(null) }}
        />
      )}
