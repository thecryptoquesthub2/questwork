...
export default function App() {
  ...
  return (
    <div style={{ backgroundColor: colors.bg, height: '100vh', width: '100vw', maxWidth: '100vw', margin: 0, padding: 0, color: colors.text, fontFamily: '-apple-system, BlinkMacSystemFont, "SF Pro Display", "Helvetica Neue", sans-serif', WebkitFontSmoothing: 'antialiased', position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, overflow: 'hidden', transition: 'background-color 0.3s ease, color 0.3s ease', boxSizing: 'border-box', display: 'flex', flexDirection: 'column' }}>

      {/* Header */}
      <div style={{ flexShrink: 0, padding: '16px 20px 12px', background: dark ? 'rgba(0,0,0,0.92)' : 'rgba(242,242,247,0.92)', backdropFilter: 'blur(30px)', WebkitBackdropFilter: 'blur(30px)', borderBottom: `1px solid ${colors.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', width: '100%', boxSizing: 'border-box', zIndex: 50 }}>
    ...
    try {
      const res = await fetch(`${API}/api/ai/pitch`, {
        ...
        body: JSON.stringify({
          ...
          user_skills: localStorage.getItem('qw_skills') || '',
          user_bio: localStorage.getItem('qw_bio') || ''
        })
      })
  ...
  const handleAIDescription = async () => {
    ...
    try {
      const res = await fetch(`${API}/api/ai/gig`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ basic_info: `${form.title} at a Web3 company, budget: ${form.pay_usdt || 'negotiable'}, duration: ${form.duration}, region: ${form.region}` })
      })
      const data = await res.json()
  ...
  const handleSubmit = async () => {
    ...
    try {
      await fetch(`${API}/api/gigs`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...form, category: finalCategory, poster_tg_id: tgUser ? String(tgUser.id) : null, poster_username: tgUser?.username || null })
      })
  ...
  return (
    <div style={{ padding: '24px 20px', width: '100%', boxSizing: 'border-box' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '13px 16px', borderRadius: '14px', background: colors.surface2, border: `1px solid ${colors.border}`, marginBottom: '20px' }}>
        <span style={{ fontSize: '15px', opacity: 0.5 }}>🔍</span>
        <input value={query} onChange={e => handleSearch(e.target.value)} placeholder={mode === 'gigs' ? 'Search gigs, companies...' : 'Search by name or username...'} style={{ flex: 1, background: 'none', border: 'none', outline: 'none', color: colors.text, fontSize: '14px', fontFamily: 'inherit' }} />
...
function ResumeBuilder({ colors, dark, tgUser, isPremium }) {
  const bio = localStorage.getItem('qw_bio') || ''
  const skills = localStorage.getItem('qw_skills') || ''
  const availability = localStorage.getItem('qw_availability') || 'Available'
...
questworkio.netlify.app
    ...
    a.href = url
    a.download = `${username}-questwork-resume.txt`
    a.click()
...
function ProfilePage({ colors, dark, tgUser, isPremium, setIsPremium, notificationsOn, setNotificationsOn, cycleTheme, theme }) {
    ...
    if (showPayment && paymentMethod === 'card') {
      const initDropin = async () => {
        try {
          ...
          const instance = await DropIn.create({ authorization: token, container: '#dropin-container' })
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      await fetch(`${API}/api/braintree/cancel`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tg_id: tgUser ? String(tgUser.id) : null })
      })
      setIsPremium(false)
      alert('Subscription cancelled.')
    } catch (err) { alert('Error cancelling subscription.') }
  }

  const displayName = tgUser ? `${tgUser.first_name} ${tgUser.last_name || ''}`.trim() : 'Guest'
  const displayUsername = tgUser?.username || 'guest'
  const profileURL = `questworkio.netlify.app/u/${displayUsername}`
